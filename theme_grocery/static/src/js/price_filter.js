...
publicWidget.registry.KzSelectedAttributes = publicWidget.Widget.extend({
    selector: ".tp-selected-attributes",
    events: {
        "click .tp-attribute": "_onClickAttribute",
    },

    _onClickAttribute(ev) {
...
publicWidget.registry.KzPriceFilter = publicWidget.Widget.extend({
    ...
    _onClickApply(ev) {
        ...
        if (this.priceSlider.result.from !== this.priceSlider.result.min) {
            this.$el.append(
                $("<input>", { type: "hidden", name: "min_price", value: this.priceSlider.result.from })
            );
        }
        if (this.priceSlider.result.to !== this.priceSlider.result.max) {
            this.$el.append(
                $("<input>", { type: "hidden", name: "max_price", value: this.priceSlider.result.to })
            );
        }
    },
});
